...
const HostPage = () => {
  ...
  const hostId = sessionStorage.getItem("adminId");
  const navigate = useNavigate();
  useEffect(() => {
    const fetchHost = async () => {
      try {
        const response = await axios.get(`http://localhost:4001/api/hosts/${hostId}`);
 ...
 const handleAdminClick = () => {
  ...
  const changePassword = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:4001/api/auth/${hostId}/change-password`, {
        oldPassword,
        newPassword,
      });
                ...
                <div className="login-wrap p-0">
                  ...
                  <form action="" className="signin-form">
                    <div className="form-group">
                      <h3 className="form-control">{host.username}</h3>
                    </div>
                    <div className="form-group">
                      <h3 className="form-control">{host.email}</h3>
                    </div>
                    {isEditing ? (
                      <div className="form-group">
                        <div className="form-group">
